...
RUN apt update \
    ...
    && apt update \
    && apt install -y sudo transmission-cli transmission-common transmission-daemon curl rar unrar zip unzip ufw iputils-ping openvpn bc sabnzbdplus python-sabyenc vim \
    ...
    && usermod -G users abc \    
    && sed -i 's/USER=/USER=abc/g' /etc/default/sabnzbdplus \
    && sed -i 's+CONFIG=+CONFIG=/etc/sabnzbd/+g' /etc/default/sabnzbdplus \
